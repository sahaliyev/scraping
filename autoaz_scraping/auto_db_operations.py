...
class AutoDbOperations:
    def __init__(self):
        self.db = DbModel()

    #  below codes are for auto.az scraping
    def auto_exists(self, unique_id, source_id):
    ...
    def insert_into_car_partial_auto(self, car):
        col_order = ('unique_id', 'year', 'engine', 'used_by_km', 'currency', 'price', 'url', 'source_id', 'loan')
    ...
    def get_unparsed_items_auto(self):
        sql = "SELECT id, url, request_count " \
              "FROM car where source_id = 2 and is_parsed=0 and request_count < 5 order by id asc"
        return self.db.get_many_items_from_db_as_dict(sql, None)

    def update_car_avto(self, car):
        col_order = ('marka', 'model', 'city', 'fuel_type', 'ban_type', 'color',
                     'engine_power', 'transmission', 'gearbox', 'description', 'date',
                     'seller', 'is_filtered')
    ...
    def update_car_auto_daily(self, car):
        col_order = ('marka', 'model', 'city', 'fuel_type', 'ban_type', 'color',
                     'engine_power', 'transmission', 'gearbox', 'description', 'date',
                     'seller')
